...
$(document).ready(function () {
 ...
 $("#navigation li a").removeClass("active");
      ...
      $("#destion").addClass("active");
    //Call show_record() function  for display description record after page loading
    show_record();
    ...
    //**************************************************************************
    //END: SAVE Description DATA
    //**************************************************************************
    
    //**************************************************************************
    //START: DISPLAY Description RECORD
    //**************************************************************************    
	...
	async: false,
        ...
        success: function(description){           
            var result = $.parseJSON(description);
            ...
            final = final +"<tr class='gradeX'><td>"+ (++slno) +"</td><td>"+v.description+"</td>"+
                           "<td><button type='button' class='btn btn-info btn-rounded btn-sm' "+
                           "id='edit_Des' value=" + v.descriptionID + ">Edit </button></td> ";
            ...
            table
                ...
                $('#basic-usage').on('click', '#edit_Des', function () { 
                    $("#Description_form")[0].reset();
                    close_message();
                    $.ajax({
                        url: "getDesReById.htm",
                        type: "POST",
                        cache: false,
                        async: false,
                        data: ({Des_id: this.value}),
                        success: function (htmlH) {                            
                            ...
                            $("#saveDescription").css("display", "none");
                            $(window).scrollTop(0);
                            charCountOfTextarea();
                        }, error: function (jqXHR, textStatus, errorThrown) {
    ...
    //**************************************************************************
    //END: DISPLAY Description RECORD
    //**************************************************************************
    ...
    $("#cancelDescription").click(function () {
        ...
        $("#saveDescription").css("display", "block");
        $('#textarea_feedback').empty();
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data === 1) {
                    ...
                    show_error_mes();
                    $("#errorDupItem").append("<strong>Sorry! &nbsp</strong> That Description already exist, Please try with different name or code.");
                    $(window).scrollTop(0);
                } else {
                    show_error_mes();
                    $("#errorDupItem").append("<strong>Error! &nbsp</strong>There is a problem while update data.");
    ...
    //**************************************************************************
    //***STAR; COUNT THE TEXTAREA LENGTH****************************************
    //************************************************************************** 
     var text_max = 200;    
    ...
    $('#description').keyup(function () {
        ...
        if (text_remaining < 0) {
            $('#textarea_feedback').html(text_remaining + ' characters length too long.').css({"color": "red"});
        } else {
            $('#textarea_feedback').html(text_remaining + ' characters remaining.').css({"color": "green"});
        }

    });

    function charCountOfTextarea() {
        $('#textarea_feedback').empty();
        var text_length = $('#description').val().length;
        var text_remaining = text_max - text_length;
        if (text_remaining < 0) {
