...
$(document).ready(function () {
    ...
    show_record();    
    //**************************************************************************
    //Start Save DISPATCH  in data base
    //**************************************************************************
    ...
    //**************************************************************************    
    //END Save DISPATCH  details in data base
    //**************************************************************************
    
    //**************************************************************************    
    //START: display DISPATCH record
    //**************************************************************************
	...
	async: false,        
        success: function(enquiry){            
            ...
            final = final +"<tr class='gradeX'>"+
                           ...
                           "id='ButBut' value=" + v.typeDispatchId + ">Edit </button></td> ";
            ...
            $('#basic-usage thead  #filterrow th').not(":eq(3)").each( function () {
                var title = $(this).text();
            ...
            table
                ...
                $('#basic-usage').on('click', '#ButBut', function () {                    
                    ...
                    close_message();
                    $.ajax({
                        url: "getDispatchMaReById.htm",
                        type: "POST",
                        cache: false,
                        async: false,
                        data: ({dispatch_id: this.value}),
                        success: function (htmlH) {                              
                            var result = $.parseJSON(htmlH);
                            $.each(result, function (k, v) {                                
                                $("#typeDispatchId").val(v.typeDispatchId);
                                $("#code").val(v.code);
                                $("#name").val(v.name);                                                              
                            });

                            $("#updateDiv").css("display", "block");
                            $("#saveDispatch").css("display", "none");
                            $(window).scrollTop(0);
    ...
    $('#saveDispatch').on('click', function() {        
        sub_mit();
    });
    
    $('#updateDispatch').on('click', function() {        
        update_dispatch();
    });
    //**************************************************************************    
    //START: DISPATCH  UPDATE 
    //**************************************************************************
    ...
    $("#cancelDispatch").click(function () {
        ...
        $("#saveDispatch").css("display", "block");
    });
    ...
    function update_dispatch() {        
        ...
        $("#successDupItem").empty();
        close_message();
        var $items = $('#typeDispatchId, #code, #name');
