...
$(document).ready(function () {
    ...
    $("#Receiverscrgbd").addClass("active");
//hide the error, success messages when load the page
    ...
    function submit_receiver() {        
        ...
        var $items = $('#receiverCode, #receiverName');
    ...
    $('#saveReceiver').on('click', function() {        
        submit_receiver();
    });
    
    $('#updateReceiver').on('click', function() {        
        update_receiver();
    });
    
    //**************************************************************************    
    //START: display RECEIVER record
    //**************************************************************************
	...
	async: false,
        ...
        success: function(receiver){            
            ...
            final = final +"<tr class='gradeX'>"+
                           "<td>"+ (slno++) +"</td>"+
                           "<td>"+v.receiverCode+"</td><td>"+v.receiverName+"</td>"+ 
                           "<td><button type='button' class='btn btn-info btn-rounded btn-sm' "+
                           "id='ButBut' value=" + v.receiverId + ">Edit </button></td> ";
            ...
            var table = $('#basic-usage').DataTable({
                autoWidth: false,
                    "columns": [
                        {title: "SL No", width: '8%'},
                        {title: "Receiver Code", width: '40%'},
                        {title: "Receiver Name", width: '40%'},                                                
            ...
            table
                ...
                $('#basic-usage').on('click', '#ButBut', function () {                    
                    ...
                    close_message();
                    $.ajax({
                        url: "getReceiverReById.htm",
                        type: "POST",
                        cache: false,
                        async: false,
                        data: ({receiver_id: this.value}),
                        success: function (htmlH) {                            
                            var result = $.parseJSON(htmlH);
                            $.each(result, function (k, v) {                                
                                $("#receiverId").val(v.receiverId);
                                $("#receiverCode").val(v.receiverCode);
                                $("#receiverName").val(v.receiverName);                                                               
                            });

                            $("#updateDiv").css("display", "block");
                            $("#saveReceiver").css("display", "none");
                            $(window).scrollTop(0);
    ...
    $("#cancelReceiver").click(function () {
        ...
        $("#saveReceiver").css("display", "block");
    });
    ...
    function update_receiver() {        
        ...
        $("#successDupItem").empty();
        close_message();
        var $items = $('#receiverId, #receiverCode, #receiverName');
